...
.config(function($routeProvider, $locationProvider, $sceProvider, $translateProvider) {
	...
	.when('/frontpage_objCfg', {
		templateUrl: ['/template/manager/sys/frontpage_objCfg.html?', Date.now()].join(''),
		controller: 'FrontpageCtrl_objCfg',
		controllerAs: 'frontpageCtrl'
	})
	.when('/frontpage_mainCfg', {
		templateUrl: ['/template/manager/sys/frontpage_mainCfg.html?', Date.now()].join(''),
